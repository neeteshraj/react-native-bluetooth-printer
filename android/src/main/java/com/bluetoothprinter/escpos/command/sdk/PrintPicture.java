...
public class PrintPicture {
   ...
   /**
     ...
     * This function prints a bitmap line by line as an image, which reduces the chance of errors.
     *
     * @param mBitmap The bitmap to be printed.
     * @param nWidth  The desired width of the printed image.
     * @param nMode   The print mode.
     * @return The byte array containing the print commands for the image.
     */
    public static byte[] POS_PrintBMP(Bitmap mBitmap, int nWidth, int nMode, int leftPadding) {
        // Convert to grayscale first, then call the function to resize the bitmap.
    ...
    /**
     * Print the image using downloaded bitmap printing
     * Receive the entire image before printing.
     *
     * @param bmp The bitmap to be printed.
     * @return The byte array containing the print commands for the image.
     */
...
*/
    ...
    public static byte[] eachLinePixToCmd(byte[] src, int nWidth, int nMode) {
        ...
        for (int i = 0; i < nHeight; ++i) {
            ...
            // GS v 0 m xL xH yL yH d1....dk Print raster bitmap using the GS command.
            data[var10 + 0] = 29;//GS
            data[var10 + 1] = 118;//v
            data[var10 + 2] = 48;//0
